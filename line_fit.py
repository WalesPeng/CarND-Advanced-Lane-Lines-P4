...
def line_fit(binary_warped):
	...
	# If cannot find the peak in histogram, try to copy one to continue the program.
	if (leftx_base == 100):
	...
	for window in range(nwindows):
		...
		if len(good_left_inds) > minpix:
			# In case of bend ahead, compensate x pos based on the previous window position
			leftx_current_next = np.int(np.mean(nonzerox[good_left_inds]) + (leftx_current - leftx_current_last) * 0.7)
		else:
			if window > 2:
				# In case of dash lane, which probably has no white pix in the window, set next window X pos the same as current one.
				leftx_current_next = leftx_current + (leftx_current - leftx_current_last)
			else:
				leftx_current_next = leftx_base

		if len(good_right_inds) > minpix:
			rightx_current_next = np.int(np.mean(nonzerox[good_right_inds]) + (rightx_current - rightx_current_last) * 0.7)
		else:
			if window > 2:
		...
		# next windows margin width minuse 5 pixel to reduce the effect of noise, as the y increasing.
		#  下一个窗口宽度减小 5
		margin -= 5
